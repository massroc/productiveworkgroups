defmodule ProductiveWorkgroups.FacilitationTest do
  ...
  describe "segment-based timer functions" do
    ...
    test "calculate_segment_duration/1 calculates correctly for 100 minute session" do
      session = %ProductiveWorkgroups.Sessions.Session{planned_duration_minutes: 100}
    ...
    test "calculate_segment_duration/1 calculates correctly for 90 minute session" do
      session = %ProductiveWorkgroups.Sessions.Session{planned_duration_minutes: 90}
    ...
    test "current_timer_phase/1 returns correct phase for scoring state" do
      session = %ProductiveWorkgroups.Sessions.Session{
        state: "scoring",
    ...
    test "current_timer_phase/1 returns nil for non-timed states" do
      assert Facilitation.current_timer_phase(%ProductiveWorkgroups.Sessions.Session{
               state: "lobby",
               current_question_index: 0
             }) == nil
    ...
    test "timer_enabled?/1 returns false for non-timed states" do
      assert Facilitation.timer_enabled?(%ProductiveWorkgroups.Sessions.Session{
    ...
    test "warning_threshold/1 returns nil for nil duration" do
      session = %ProductiveWorkgroups.Sessions.Session{planned_duration_minutes: nil}
